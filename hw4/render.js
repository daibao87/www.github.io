...
export function list(posts, user, specificUser = null) {
  let list = [];
  ...
  const header = specificUser
    ? `Posts by ${specificUser}`
    : 'All Posts';
  ...
  ${specificUser ? '' : '<p>View posts by user: ' + posts.map(p => `<a href="/list/${p.username}">${p.username}</a>`).join(' | ') + '</p>'}
  `;
